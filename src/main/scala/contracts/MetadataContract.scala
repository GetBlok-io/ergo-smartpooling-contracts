...
object MetadataContract {
  ...
  def getMetadataScript: String = {
     ...
     * -- Element 3+ are not looked at or verified. This information may be stored and parsed according to the smart pool owner.
     *
  ...
  /**
   * Generates a redeposit Metadata Box with properly assigned registers
   * @param ctx Context as Blockchain Context
   * @param metadataBox Metadata Box to obtain info from.
   */
  def getRawMetaDataInfo(ctx: BlockchainContext, metadataBox: InputBox): (Coll[(Coll[Byte], Long)], Coll[(Coll[Byte], Coll[Byte])], Coll[(Coll[Byte], Int)], Coll[Int], Coll[(Coll[Byte], Coll[Byte])]) = {

    val lastConsensus = metadataBox.getRegisters.get(0).getValue.asInstanceOf[Coll[(Coll[Byte], Long)]]
    val memberCollection = metadataBox.getRegisters.get(1).getValue.asInstanceOf[Coll[(Coll[Byte], Coll[Byte])]]
    val poolFees = metadataBox.getRegisters.get(2).getValue.asInstanceOf[Coll[(Coll[Byte], Int)]]
    val poolInfo = metadataBox.getRegisters.get(3).getValue.asInstanceOf[Coll[Int]]
    val poolOperators = metadataBox.getRegisters.get(4).getValue.asInstanceOf[Coll[(Coll[Byte], Coll[Byte])]]

    (lastConsensus, memberCollection, poolFees, poolInfo, poolOperators)
    }

  def getCurrentEpoch(metadataBox: InputBox): Int ={
    val poolInfo = metadataBox.getRegisters.get(3).getValue.asInstanceOf[Coll[Int]]
    poolInfo(0)
  }

  def getCurrentEpochHeight(metadataBox: InputBox): Int ={
    val poolInfo = metadataBox.getRegisters.get(3).getValue.asInstanceOf[Coll[Int]]
    poolInfo(1)
  }

  def getCreationHeight(metadataBox: InputBox): Int ={
    val poolInfo = metadataBox.getRegisters.get(3).getValue.asInstanceOf[Coll[Int]]
    poolInfo(2)
  }

  def getLastConsensus(metadataBox: InputBox): Coll[(Coll[Byte], Long)] ={
    val lastConsensus = metadataBox.getRegisters.get(0).getValue.asInstanceOf[Coll[(Coll[Byte], Long)]]
    lastConsensus
  }

  def getMemberList(metadataBox: InputBox): Array[(Coll[Byte], Address)] ={
    val memberCollection = metadataBox.getRegisters.get(1).getValue.asInstanceOf[Coll[(Coll[Byte], Coll[Byte])]]
    val memCollWithAddress = memberCollection.toArray.map{
      (member: (Coll[Byte], Coll[Byte])) =>
        (member._1, Address.create(member._2.toArray.toString))

    }
    memCollWithAddress
  }

  def getPoolFees(metadataBox: InputBox): Array[(Coll[Byte], Int)] ={
    val poolFees = metadataBox.getRegisters.get(2).getValue.asInstanceOf[Coll[(Coll[Byte], Int)]]
    poolFees.toArray
  }
